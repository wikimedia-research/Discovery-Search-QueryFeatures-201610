...
<!-- These should be rendered as <p class="caption">...</p> because of the 'fig_caption: yes' YAML above. If you want italicized captions, it is better to set '.caption { font-size: 10pt; font-style: italic; }' in style.css -->
...
Because AUC is too close to 0.5, and the class error for zero click is too large, we don't think query features have large enough predicting power on clickthrough rate, and thus we are not reporting variable importance here (see appendix below).

### PaulScore

**Linear Regression: ** We fit linear regression models (lambda of elasticnet penalty is very close to 0) for PaulScore when scoring factor equals 0,1, 0.5 and 0.9. The explained deviance by models are very small, less than 0.3%, and R squared ranges from 0.005 to 0.009. Therefore, we do not think query features have enough predictive power on PaulScore, and thus we are not reporting variable importance here.

## Appendix

![**Figure 6**: Needs a caption.](index_files/figure-html/var_imp_ctr.png)

![**Figure 7**: Needs a caption too.](index_files/figure-html/mda_logitcoef_ctr.png)
